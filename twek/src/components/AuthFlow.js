...
const AuthFlow = () => {
  ...
  const handleMobileSubmit = async (e) => {
    ...
    try {
      const result = await phoneOtpAPI.sendOTP(`+91${mobile}`);
      
      console.log('Phone OTP API Response:', result);
      
      if (result.success) {
        setAuthData(prev => ({ ...prev, mobile: `+91${mobile}` }));
      ...
      if (error.status === 400) {
        if (error.message && error.message.includes('duplicate key')) {
          ...
          setErrorMessage('Invalid phone number or request. Please check and try again.');
        }
  ...
  const handleMobileOtpSubmit = async (e) => {
    ...
    const otpError = validateOtp(mobileOtp);
    if (otpError) {
    ...
    try {
      const result = await phoneOtpAPI.verifyOTP(authData.mobile, mobileOtp);
      ...
      if (result.success) {
        ...
        setSuccessMessage(result.message || 'Phone verified successfully!');
      ...
      setErrorMessage('Failed to verify OTP. Please check your code and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      const result = await emailOtpAPI.sendOTP(email, authData.mobile);

      if (result.success) {
        setAuthData(prev => ({ ...prev, email: email }));
        setCurrentStep('emailOtp');
        setSuccessMessage(result.message || 'Email OTP sent successfully!');
        startOtpTimer(60);
      } else {
        setErrorMessage(result.message || 'Failed to send OTP. Please try again.');
      }
      ...
      if (error.status === 400) {
        if (error.message && error.message.includes('duplicate key')) {
          ...
          setErrorMessage('Invalid email or request. Please check and try again.');
        }
      } else if (error.status === 429) {
        setErrorMessage('Too many requests. Please wait a moment and try again.');
      } else {
  ...
  const handleEmailOtpSubmit = async (e) => {
    ...
    const otpError = validateOtp(emailOtp);
    if (otpError) {
      setErrors({ emailOtp: otpError });
      return;
    }
    ...
    try {
      const result = await emailOtpAPI.verifyOTP(authData.email, authData.mobile, emailOtp);
  ...
  const resendOtp = async () => {
    ...
    try {
      let result;
      if (currentStep === 'mobileOtp') {
        result = await phoneOtpAPI.sendOTP(authData.mobile);
      } else if (currentStep === 'emailOtp') {
        result = await emailOtpAPI.sendOTP(authData.email, authData.mobile);
      }
      ...
      setErrorMessage('Failed to resend OTP. Please check your connection and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // If auth is completed, show success message
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={handleMobileSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-lg font-medium hover:from-blue-700 hover:to-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
              >
                {isLoading ? (
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-blue-600 font-semibold">{authData.mobile}</p>
            </div>
            ...
            <form onSubmit={handleMobileOtpSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading || !mobileOtp || validateOtp(mobileOtp)}
                className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-lg font-medium hover:from-blue-700 hover:to-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
              >
                {isLoading ? (
                  <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-blue-600 font-semibold">{authData.email}</p>
            </div>
            ...
            <form onSubmit={handleEmailOtpSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading || !emailOtp || validateOtp(emailOtp)}
                className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-lg font-medium hover:from-blue-700 hover:to-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
              >
                {isLoading ? (
                  <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
        ...
        return (
          <div className="space-y-6">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading || !password || !confirmPassword || validatePassword(password) || password !== confirmPassword}
